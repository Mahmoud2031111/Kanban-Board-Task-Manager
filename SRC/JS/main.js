...
function renderBoard() {
    for (const col of COLUMNS) {
        const list = $(col.listId);
        const countEl = $(col.countId);
...
function openModal(mode, taskId) {
    ...
    const submitLabel = $("modal-submit-label");
    form.reset();
    editingTaskId = null;
    if (mode === "edit" && taskId) {
...
function init() {
    const modalEl = $("crud-modal");
    modalInstance = new Modal(modalEl, {
        backdropClasses: "bg-gray-900/50 fixed inset-0 z-40",
    });
    $("add-task-btn").addEventListener("click", () => openModal("create"));
    $("modal-close-btn").addEventListener("click", () => modalInstance.hide());
    $("modal-cancel-btn").addEventListener("click", () => modalInstance.hide());
